...
while run:
    ...
    if player_x +size/2 > width:
       player_x = width - size/2
    elif player_x -size/2 < 0:
       player_x = 0 +size/2

    if player_y +size/2 > height:
       player_y = height - size/2
    elif player_y -size/2 < 0:
       player_y = 0+size/2
    
    #eatfood
    for x in list(food):
       if player_x-size/2-2.5 < x < player_x +size/2+2.5 and player_y-size/2-2.5 < food[x] < player_y +size/2+2.5:
          size+=1
          score+=1
          totalfood-=1
          del food[x]
    ...
    for x in enemy:
       if player_x-size/2-7.5 < x.posx < player_x +size/2+7.5 and player_y-size/2-7.5 < x.posy < player_y +size/2+7.5:
          ...
          continue
    for x in bullet:
       if player_x-size/2-2.5 < x.posx < player_x +size/2+2.5 and player_y-size/2-2.5 < x.posy < player_y +size/2+2.5:
          size-=3/2
          bullet.remove(x)
          continue
    if size < 1:
        run = False
    if size < 100 and sizemod > 1:
      size=300
      sizemod-=1
    ...
    pygame.draw.circle(wn, (0,lol,0), (player_x,player_y), size/2)
    for x in food:
       pygame.draw.circle(wn, (random.uniform(0,200),random.uniform(0,200),random.uniform(0,200)), (x,food[x]), 5/sizemod)
    for x in enemy:
       x.attacktimer += 1
       x.movement += 1
       ...
       if x.posy> height:
            ...
            x.posy = 0
       pygame.draw.circle(wn, (255,0,0), (x.posx,x.posy), 15/sizemod)
    ...
    for x in player:
       ...
       if x.posy + x.size> height:
            ...
            x.posy = 0+x.size
       pygame.draw.circle(wn, (0,255,0), (x.posx,x.posy), x.size/sizemod)
    ...
    for x in bullet:
       ...
       if x.posy > height:
          ...
          x.vely *= -1
       pygame.draw.circle(wn, (255,255,0), (x.posx,x.posy),5/sizemod)
    #eyeballs
    if player_x_vel == 0 and player_y_vel == 0 or player_x_vel == 0 and player_y_vel < 0:
       pygame.draw.circle(wn, (255,255,255), (player_x-size/2/2,player_y-size/2/2.5), size/2/5)
       pygame.draw.circle(wn, (255,255,255), (player_x+size/2/2,player_y-size/2/2.5), size/2/5)
       pygame.draw.circle(wn, (0,0,0), (player_x-size/2/2,player_y-size/2/2.5), size/2/10)
       pygame.draw.circle(wn, (0,0,0), (player_x+size/2/2,player_y-size/2/2.5), size/2/10)
    ...
    #diagonals
    if player_x_vel < 0 and player_y_vel < 0:
       pygame.draw.circle(wn, (255,255,255), (player_x-size/2/4,player_y-size/2/1.5), size/2/5)
       pygame.draw.circle(wn, (255,255,255), (player_x-size/2/1.5,player_y-size/2/4), size/2/5)
       pygame.draw.circle(wn, (0,0,0), (player_x-size/2/4,player_y-size/2/1.5), size/2/10)
       pygame.draw.circle(wn, (0,0,0), (player_x-size/2/1.5,player_y-size/2/4), size/2/10)
    if player_x_vel > 0 and player_y_vel > 0:
       pygame.draw.circle(wn, (255,255,255), (player_x+size/2/4,player_y+size/2/1.5), size/2/5)
       pygame.draw.circle(wn, (255,255,255), (player_x+size/2/1.5,player_y+size/2/4), size/2/5)
       pygame.draw.circle(wn, (0,0,0), (player_x+size/2/4,player_y+size/2/1.5), size/2/10)
    ...
    text_surface = font.render(f'{score}', True, pygame.Color('orange'))
